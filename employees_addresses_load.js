/**
 * K6 Load Test: /employees/addresses/
 * 
 * PROFILES:
 * - profile_15: ~15 RPS
 * - profile_30: ~30 RPS
...
export function setup() {
  ...
  console.log(`[SETUP] Target: ${BASE_URL}/employees/addresses/`);
}

export default function () {
  const params = { headers: getAPIHeaders() };
  
  const response = http.get(`${BASE_URL}/employees/addresses/`, params);
  ...
  if (response.status !== 200) {
    console.log(`❌ employees/addresses: status ${response.status} - ${response.status_text}`);
  }
